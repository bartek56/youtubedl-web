...
def createPlaylistForSandisk(folders, listName):
    PATH=os.path.abspath(os.getcwd())
...
def createPlaylistForMediaserver(folders, listName):
...
def createTopOfMusicGarmin():
        ...
        mp3_files = sorted(mp3_files, key= lambda f: (get_date(f)),
#                                                      get_tracknumber(f)),
                           reverse=True)
...
def createTopOfMusic(numberOfSongs):
        mp3_files = []
...
def main(argv):
    ...
    if isMediaserver:
        ...
        for x in listDir:
            createPlaylistForMediaserver([x], x)
        createTopOfMusic(100)
    ...
    if isSandisk:
        ...
        createPlaylistForSandisk(folders,"taniec")
        ...
        createPlaylistForSandisk(folders,"trening")
        ...
        folders.append("Rock-Electronic")
        createPlaylistForSandisk(folders,"praca")
